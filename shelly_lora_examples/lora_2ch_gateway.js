...
Shelly.addEventHandler(function (event) {
  ...
  if (typeof event === 'object' &&
    ...
    if (typeof decryptedMessage === "undefined") {
      ...
      if (decryptedMessage.substring(0, 1) === "C") {
      ...
      if (decryptedMessage.substring(0, 1) === "D") {
        if (decryptedMessage.substring(1, 2) === "O") {
          // turn on the light
...
Shelly.addStatusHandler(function (status) {
  ...
  // look for the status message when the CH1 switch was changed
  if (typeof status === "object" && status.component === "switch:0" && status.delta) {
    if (status.delta.source !== "loopback") { // prevent lora change triggering a return message
  ...
  // look for the status message when the CH2 switch was changed
