...
VCDevice0State = Virtual.getHandle("boolean:200");
VCDevice1State = Virtual.getHandle("boolean:201");
VCDevice0Power = Virtual.getHandle("number:200");
VCDevice1Power = Virtual.getHandle("number:201");
...
Shelly.addEventHandler(function (event) {
  ...
  if (typeof event === 'object' &&
      ...
      if (typeof decryptedMessage === "undefined") {
        ...
        // Device 0 state
        if (decryptedMessage.substring(0, 1) === "G") {
          if (decryptedMessage.substring(1, 2) === "O") {
            // Device 0 is ON
            VCDevice0State.setValue(true);
          }
          if (decryptedMessage.substring(1, 2) === "F") {
            // Device 0 is OFF
            VCDevice0State.setValue(false);
          }
        }
        
        // Device 1 state
        if (decryptedMessage.substring(0, 1) === "H") {
          if (decryptedMessage.substring(1, 2) === "O") {
            // Device 1 is ON
            VCDevice1State.setValue(true);
          }
          if (decryptedMessage.substring(1, 2) === "F") {
            // Device 1 is OFF
            VCDevice1State.setValue(false);
          }
        }
        
        // Power consumption data
        if (decryptedMessage.substring(0, 1) === "I") {
          let arr = decryptedMessage.split("|");
          // Update the power figures
          VCDevice0Power.setValue(parseFloat(arr[1]));
          VCDevice1Power.setValue(parseFloat(arr[2]));
        }
