...
Shelly.addEventHandler(function (event) {
  ...
  if (typeof event === 'object' &&
    ...
    if (typeof decryptedMessage === "undefined") {
      ...
      // CH1 Switch message is received
      if (decryptedMessage.substring(0, 1) === "A") {
      ...
      // CH2 Switch message is received
...
Shelly.addStatusHandler(function (status) {
  ...
  // look for the status message when the CH1 switch was changed by pressing the button
  if (typeof status === "object" && status.component === "switch:0" && status.delta) {
    if (status.delta.source === "button") { // prevent lora change triggering a return message
      if (status.delta.output) {
        // output got turned on
        sendMessage("CO");
        console.log("CH1 on message sent");
      } else {
        sendMessage("CF");
        console.log("CH1 off message sent");
      }
    }  
  }
  
  // look for the status message when the CH2 switch was changed by pressing the button
  if (typeof status === "object" && status.component === "switch:1" && status.delta) {
    if (status.delta.source === "button") { // prevent lora change triggering a return message
      if (status.delta.output) {
        // output got turned on
        sendMessage("DO");
        console.log("CH2 on message sent");
      } else {
        sendMessage("DF");
        console.log("CH2 off message sent");
      }
    }  
  }
});